...
function getAllStores($conn, $id){
    $Query = "SELECT * FROM tbl_stores WHERE owner = '$id'";
    $Results    = mysqli_query($conn,$Query);
    $ListArray = array();
...
function getStores($conn){
    $Query = "SELECT * FROM tbl_stores WHERE status = 'Open'";
    $Results    = mysqli_query($conn,$Query);
    $ListArray = array();
    ...
    if (mysqli_num_rows($Results) > 0) 
    {
        while($record = mysqli_fetch_assoc($Results)) 
        {
            ...
            if($LoginState)
            {
                ...
                {
                    $CartContent = '<button type="button" style="display:none;background-color: #B885C1;outline: none;border: none;" class="btn btn-secondary btn-sm" id="bt_cart_'.$record["id"].'" onclick="add_to_cart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\',\''.$order_type.'\')" >Add to cart</button>
                    ...
                    $CartContent = '<button type="button" style="background-color: #B885C1;outline: none;border: none;" class="btn btn-secondary btn-sm" id="bt_cart_'.$record["id"].'" onclick="add_to_cart(\''.$store.'\',\''.$record["id"].'\',\''.$_SESSION["uid"].'\',\''.$order_type.'\')" >Add to cart</button>
...
function getStoreRequest($conn){
    $GetEvents = "SELECT s.*, u.first_name, u.last_name FROM tbl_stores AS s INNER JOIN tbl_login AS u ON s.owner = u.id WHERE s.status = 'Requested';    ";
